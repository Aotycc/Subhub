...
// 检查是否为完整的Clash配置
function isCompleteClashConfig(content) {
    // 检查是否包含完整的Clash配置结构
...
export async function handleClashRequest(request, env) {
    try {
        ...
        // 新增参数：useOriginal=true 表示保留原始配置
        const useOriginal = url.searchParams.get('useOriginal') === 'true';
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(`Failed to fetch from ${directUrl}: ${response.status}`);
            }
            originalContent = await response.text();
        } catch (error) {
            console.error('Error fetching subscription:', error);
            return new Response('Failed to fetch subscription: ' + error.message, { status: 500 });
        }
        
        // 检测是否为完整的Clash配置
        const isCompleteClash = isCompleteClashConfig(originalContent);
        
        // 如果是完整的Clash配置且用户选择保留原始配置
        if (isCompleteClash && useOriginal) {
            console.log('Using original complete Clash configuration');
        ...
        try {
            ...
            return new Response('Error parsing nodes: ' + error.message, { status: 500 });
        }
